############################################
# FUNDAMENTOS DE ANALITICA DE DATOS EN R
...
dim(cafe_df)  # Para conocer la dimension del dataframe - Nro filas y columnas
###########################################################
# Ahora, podemos ver la informacion resumen de todo el dataframe
...
# informacion del dataframe con varias variables
tapply(cafe_df$Area..ha.,cafe_df$Produccion..ton.,summary)
###################################################
# tambien algunos valores estadisticos 
...
str(cafe_df)
#La funcion str() nos muestra la estructura de este objeto, 
###############################################
...
#Listar contenido del dataframe, parte inicial, final y por seleccion especifica
head(cafe_df)
tail(cafe_df)
cafe_df[1:3,]
cafe_df[3:6,]
...
cafe_df[,2] #lista los valores de la columna 2
cafe_df[2,] #seleccion de los valores de la segunda fila
cafe_df[100,] #seleccion de los valores de la  fila 100
cafe_df
cafe_df[,2] #lista los valores de la columna 2
cafe_df[2,] #seleccion de los valores de la segunda fila
cafe_df[,2]
...
#Para eliminar filas definiremos un vector de tipo logico en el que 
# indicaremos para cada fila si sera eliminada o no:
...
#mediante la funciOn `rbind()` (acronimo de _rowbind_, pegar por filas):
...
#Añadimos la nueva fila a 'Cafe2' con 'rbind()'
...
# se puede utilizar la funciOn subset(x, subset, select)
# El parametro x sirve para indicar el marco de datos o dataframe original
# el parametro subset sirve para colocar la condicion 
# el parametro select sirve para quedarnos slo con algunas de las variables
cafe_df
...
# si existen datos NULOS (NA), se tendra la respuesta TRUE (TRUE equivale a 1)
...
moda=function(var){
  ...
  names(valor)}
# ahora se llama a la funcion para que entregue el valor
...
#########################################
# seguimos estudiando el comportamiento o distribuciïn
# de los datos o la informacion con graficos de dispersion
# Grafico de dispersion del comportamiento 
# de la produccion versus Rendimiento
...
#################     
# Grafico de dispersion del comportamiento 
# de la produccion nacional versus Rendimiento
# permite entender o descubrir patrones en los datos
...
# coeficiente de correlacion entre La produc nacional y el rendimiento
# hallar la correlacion entre las variables por correlacion de pearson
# indicamos que utiliza las variables con valores descartando los valores nulos que existan 
...
################## 
# coeficiente de correlacion de Pearson
# coeficiente de correlacion entre el area nacional y el rendimiento
...
#install.packages("ggplot2") #Se instala 1 sola vez
library(tidyverse)
# Especificar el dataframe es el primer argumento en la funcion ggplot
...
# Grafico de dispersion: Area Nacional vs Rendimiento
ggplot(cafe_df) + 
...
################
################
# Grafico de cajas: Produccion Nacional vs Rendimiento
ggplot(cafe_df) + 
  geom_boxplot(aes(x = Produccion.Nacional..ton., y = Rendimiento..ha.ton., color = 'Anio'))
################
# Grafico de cajas: Area Nacional vs Rendimiento
ggplot(cafe_df) + 
...
# analisis del comportamiento, dispersion mediante colores, agrupados por años
ggplot(cafe_df) + 
...
ggplot(cafe_df) + 
  geom_point(aes(x = Produccion.Nacional..ton., y = Rendimiento..ha.ton., color = 'Anio'))
################
# Grafico de dispersion con jitter: Produccion Nacional vs Rendimiento
ggplot(cafe_df) + 
  geom_jitter(aes(x = Produccion.Nacional..ton., y = Rendimiento..ha.ton., color = 'Anio'))
#####################
# otra forma con grafico de dispersion sin representar los aaos
#data(cafe_df)
ggplot(cafe_df, aes(x = Produccion.Nacional..ton., y = Rendimiento..ha.ton., color ="red")) + 
  geom_point()
# otra forma con grafico de lineas sin representar los años
...
ggplot(cafe_df, aes(x = Produccion.Nacional..ton., y = Rendimiento..ha.ton., color ="red")) + 
  geom_point()
#############################
# comparar con el Grafico de dispersion  Produccion Nacional vs Rendimiento con años
ggplot(cafe_df) + 
  geom_jitter(aes(x = Produccion.Nacional..ton., y = Rendimiento..ha.ton., color = 'Anio'))

##############################################
###############################################
#Grafica de dispersion del Area Nacional versus Produccion Nacional
...
# CREAMOS EL MODELO DE REGRESION LINEAL - PRODUC. NACIONAL VERSUS AREA NACIONAL
modelo_regresion_lineal=lm( Produccion.Nacional..ton.~ Area.Nacional..ha.,data=cafe_df, na.action=na.exclude)
#####################################
#####################################
# A continuaciOn se describen los parametros del modelo anterior
summary(modelo_regresion_lineal)
######################################
# Construccion de la grafica Area Nal. versus Prod. Nacional 
...
##########################################
##########################################
# MODELO DE REGRESION LINEAL - Area Nacional versus Produccion Nacional
# Ahora se representa de dispersion con el modelo de regresion lineal en color azul
# la linea recta de color azul representa el modelo.
Grafica_AreaNal_ProdNal_1 + geom_point()+geom_smooth(method="lm", colour ="Blue")
########################################################
################################################################
###############################################
#Grafica de dispersion del Area Nacional versus Rendimiento
...
############################################
## CREAMOS EL MODELO DE REGRESIoN LINEAL DE RENDIMIENTO VERSUS AREA NACIONAL
...
# Ahora se representa GRAFICA 2 con el modelo de regresion lineal en color azul
# la linea recta de color azul representa el modelo.
Grafica_AreaNal_Rendim_1 + geom_point()+geom_smooth(method="lm", colour ="Blue")
########################################################
################################################################
# Analisis de los datos mediante otros tipos de graficas
